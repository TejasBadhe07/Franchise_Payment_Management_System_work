...
from database import db, FinancialAccount, Panel, Expense, BalanceHistory, SubmissionHistory

# Create a Blueprint for worker dashboard routes
worker_bp = Blueprint('worker', __name__)

@worker_bp.route('/worker_dashboard')
def worker_dashboard():
    ...
    # Fetch the last submission for balance history
    last_balance = BalanceHistory.query.filter_by(username=username).order_by(BalanceHistory.timestamp.desc()).first()

    old_balance = last_balance.old_balance if last_balance else 0
    old_points = last_balance.old_points if last_balance else 0
    profit_or_loss = last_balance.profit_or_loss if last_balance else 0
    plus_or_minus = last_balance.plus_or_minus if last_balance else 0
    ...
    return render_template('worker_dashboard.html', accounts=accounts, panels=panels, expenses=expenses,
                           ...
                           old_balance=old_balance, old_points=old_points,
                           profit_or_loss=profit_or_loss, plus_or_minus=plus_or_minus)

@worker_bp.route('/add_accounts', methods=['POST'])
def add_account():
...
@worker_bp.route('/delete_account/<int:account_id>', methods=['POST'])
...
@worker_bp.route('/update_balance/<int:account_id>', methods=['POST'])
...
@worker_bp.route('/update_points/<string:panel_name>', methods=['POST'])
...
@worker_bp.route('/add_panel', methods=['POST'])
def add_panel():
...
@worker_bp.route('/delete_panel/<int:panel_id>', methods=['POST'])
...
@worker_bp.route('/add_expense', methods=['POST'])
def add_expense():
...
@worker_bp.route('/delete_expense/<int:expense_id>', methods=['POST'])
...
@worker_bp.route('/add_points/<string:panel_name>', methods=['POST'])
...
@worker_bp.route('/withdraw_points/<string:panel_name>', methods=['POST'])
...
@worker_bp.route('/submit_data', methods=['POST'])
def submit_data():
...
@worker_bp.route('/get_last_submission_time', methods=['GET'])
